...
For each significant RAG pipeline recommendation produced by the parallel sub-agents:
...
> This prevents plausible-but-wrong RAG pipeline recommendations from reaching the final output. The 3-vote panel eliminates single-point hallucination without requiring unanimity.

## Cross-System Consistency Validator

After all parallel agents (Ingestion Agent, Embedding Evaluator, Vector DB Tester, Diagnostics Agent) complete, but **before** synthesis:

Run one **Consistency Validator Agent** with all parallel outputs that:
- Flags any pair of recommendations that logically contradict each other
  *(example: the Embedding Evaluator recommends a high-dimensional model for precision while the Vector DB Tester recommends a low-dimensional index configuration optimized for speed that would degrade quality at that dimensionality)*
...
2. **`SEQUENCING_REQUIRED` items**: Re-order the unified RAG pipeline recommendation so prerequisites appear before the steps that depend on them
