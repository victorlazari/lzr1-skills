...
Accented characters and special symbols can print incorrectly on thermal receipt printers if the correct code page is not selected and the string is not encoded with the matching codec. This document provides a complete guide to selecting and using the correct code pages on the Tomate POS-80.
...
The `ESC t n` command is used to switch between different hardware code pages built into the printer's ROM.
...
The parameter `n` selects the active code page. The table below lists the relevant pages supported by the Tomate POS-80:
...
Since Tomate is a brand primarily distributed in Brazil, Portuguese character encoding is the most common integration requirement.
...
1. Initialize the printer using `ESC @` (`1B 40`).
2. Select the **WPC1252** code page using `ESC t 16` (`1B 74 10`).
...
If WPC1252 is not available or supported by a specific legacy software driver, you can use the native **PC860** Portuguese hardware page by sending `ESC t 3`. Below is the character translation map for PC860:
...
## References
- ZKTECO Colombia, *POS-80-Series Printer Programmer Manual (ESC t)*, Page 28, https://zktecocolombia.com/wp-content/uploads/2025/08/Manual-de-Progamacion.pdf
