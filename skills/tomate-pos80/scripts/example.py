...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Tomate POS-80 Thermal Printer Integration Example")
    parser.add_argument("--ip", help="IP address of the printer (e.g., 192.168.1.100)")
    parser.add_argument("--port", type=int, default=9100, help="Port of the printer (default: 9100)")
    parser.add_argument("--dry-run", action="store_true", help="Run in dry-run mode (output bytes to console)")
    parser.add_argument("--confirm", action="store_true", help="Confirm sending data to the physical printer")

    args = parser.parse_args()

    if args.ip and not args.dry_run and not args.confirm:
        print("WARNING: You are about to send raw bytes to a physical printer.")
        print("Please use --confirm to proceed or --dry-run to test locally.")
        sys.exit(1)

    print_receipt(ip=args.ip, port=args.port, dry_run=args.dry_run)
